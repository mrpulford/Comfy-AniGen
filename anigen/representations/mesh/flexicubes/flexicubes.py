...
try:
    ...
    def check_tensor(x, shape, throw=False):
        ...
        if len(shape) != x.ndim:
            if throw:
                raise ValueError(f"Expected tensor rank {len(shape)}, got {x.ndim}")
            return False
        for dim, expected in enumerate(shape):
            if expected is not None and x.shape[dim] != expected:
                if throw:
                    raise ValueError(f"Expected tensor shape {shape}, got {tuple(x.shape)}")
                return False
        return True
