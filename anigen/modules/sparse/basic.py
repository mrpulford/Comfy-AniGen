...
elif BACKEND == 'trispconv':
    SparseTensorData = importlib.import_module('trispconv.pytorch').SparseConvTensor
    ...
    def __init__(self, *args, **kwargs):
        ...
        if method_id == 0:
            ...
            if BACKEND == 'torchsparse':
                ...
                spatial_shape = list(coords.max(0)[0] + 1)[1:]
                self.data = SparseTensorData(feats.reshape(feats.shape[0], -1), coords, spatial_shape, shape[0])
                self.data._features = feats
        elif method_id == 1:
    ...
    def replace(self, feats: torch.Tensor, coords: Optional[torch.Tensor] = None) -> 'SparseTensor':
        ...
        if BACKEND == 'torchsparse':
            ...
            new_data = SparseTensorData(
                ...
                self.data.spatial_shape,
                self.data.batch_size,
                indice_dict=self.data.indice_dict,
            )
            new_data._features = feats
